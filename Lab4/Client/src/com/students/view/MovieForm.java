...
public class MovieForm extends javax.swing.JFrame {
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        ...
        if (ldt != null && duration != null && budget != null) {
            ...
            movie.setCharacters(currentCharacters.stream().map(Персонаж::getId).collect(Collectors.toList()));
            movie.setCountries(countriesTestField.getText());
            movie.setDirectors(currentDirectors.stream().map(Director::getId).collect(Collectors.toList()));
